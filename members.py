...
while count < 100:
    ...
    root = Tk()
    root.geometry("600x600+390+190")

    Label(root, text="\nThe Number of Members from: " + states[count] + " is?\n", font=18).grid(row=2, column=0)

    v = StringVar()

    for i in range(5):
        b = Radiobutton(root, text=short_list[i], variable=v, command=ShowChoice, value=short_list[i], font=18,
                        indicatoron=0)
